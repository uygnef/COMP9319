...
int main(int argc, char* argv[]) {
    ...
    if(!_file)
        ...
        map<char, int> count_map = read_last_line(index_file);
    }



    for( map<char,int>::iterator it = count_map.begin(); it != count_map.end(); ++it)
        cout<<"key: "<<int(it->first) <<" value: "<<it->second<<endl;
...
int read_last_line(string index_file) //copy from http://stackoverflow.com/questions/11876290/c-fastest-way-to-read-only-last-line-of-text-file
{
